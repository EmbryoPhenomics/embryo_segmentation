...
# Source: https://github.com/niecongchong/HRNet-keras-semantic-segmentation/blob/master/model/seg_hrnet.py
...
def final_layer(x, classes=1):
    ...
    x = Conv2D(classes, 1, use_bias=False, kernel_initializer='he_normal', dtype=tf.float32)(x)
    x = BatchNormalization(axis=3)(x)
    x = Activation('sigmoid', name='Classification', dtype=tf.float32)(x)
    return x

def HRNetV2(input_shape, n_classes):
    ...
    out = final_layer(x, classes=n_classes)
...
if __name__ == '__main__':
    model = HRNetV2(input_shape=(512, 512, 3), n_classes=1)
    model.summary()